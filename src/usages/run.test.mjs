...
beforeEach(() => { repo = mkdtempSync(join(tmpdir(), 'cg-usg-run-')); });
...
describe('run() happy path', () => {
  it('writes manifest/nodes/edges and computes symbol→symbol USES', async () => {
    ...
    const out = join(repo, '.kg-cache', 'usages');
    ...
    // useFoo uses foo → 2 symbols involved, 1 edge.
    expect(manifest.counts).toEqual({ symbols: 2, edges: 1 });

    const edges = readLines(join(out, 'edges.jsonl'));
    expect(edges).toContainEqual({
      id: 'edge:sym:src/b.ts#useFoo:USES:sym:src/a.ts#foo',
      type: 'USES', from: 'sym:src/b.ts#useFoo', to: 'sym:src/a.ts#foo', properties: {},
    });

    // Involved Symbol nodes are reused verbatim from the symbols layer.
    const nodes = readLines(join(out, 'nodes.jsonl'));
    expect(nodes).toContainEqual(symbolNode('src/a.ts', 'foo', true));
    expect(nodes).toContainEqual(symbolNode('src/b.ts', 'useFoo', true));
  });
...
describe('run() exit codes & flags', () => {
  ...
  it('honors --symbols / --inventory pointing at custom dirs', async () => {
    ...
    writeFileSync(join(symdir, 'nodes.jsonl'),
      [symbolNode('src/a.ts', 'foo', true), symbolNode('src/b.ts', 'g', true)]
        .map((n) => JSON.stringify(n)).join('\n'));
    writeFileSync(join(symdir, 'manifest.json'), JSON.stringify({ basedOnSnapshot: 'snapshot:x:y' }));
    src('src/a.ts', 'export function foo() {}\n');
    src('src/b.ts', "import { foo } from './a';\nexport function g() { return foo(); }\n");

    const code = await run([...outArg(), '--inventory', inv, '--symbols', symdir]);
    expect(code).toBe(0);
    const manifest = JSON.parse(readFileSync(join(repo, '.kg-cache', 'usages', 'manifest.json'), 'utf8'));
