...
describe('docs CLI — model-written descriptions', () => {
  const row = {
  ...
  it('renders cached descriptions INSIDE the generated markers, labelled', async () => {
    ...
    // Inside the block that `docs` owns, so a re-run replaces it rather than
    // leaving a stale generated sentence behind.
    const inner = agents.slice(agents.indexOf(BEGIN_MARKER), agents.lastIndexOf(END_MARKER));
    expect(inner).toContain('Shared primitives every other domain builds on.');

    const corePage = read(docsDir(), 'domains', 'core.md');
    expect(corePage).toContain('Generated by `fake-model`');
  });

  it('is still idempotent with descriptions present', async () => {
    writeDescriptions(cache, 'domain', [row]);
    await run(['--repo-root', repo, '--cache', cache]);
    const before = read(repo, 'AGENTS.md');
    await run(['--repo-root', repo, '--cache', cache]);
    expect(read(repo, 'AGENTS.md')).toBe(before);
  });

  it('changes nothing when no descriptions have been generated', async () => {
    await run(['--repo-root', repo, '--cache', cache]);
    expect(read(repo, 'AGENTS.md')).not.toContain('Generated by');
  });
});
