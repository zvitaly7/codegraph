...
describe('renderDocs — model-written descriptions', () => {
  const store = (rows) => ({ get: (id) => rows[id] });
  const coreRow = {
    kind: 'domain',
  ...
  it('renders nothing extra when no descriptions exist', () => {
    const agents = renderDocs(g, { descriptions: store({}) }).find((p) => p.kind === 'agents').content;
    expect(agents).toBe(page('agents').content);
    expect(agents).not.toContain('model-written');
  });

  it('puts domain descriptions in their own labelled AGENTS.md section', () => {
    const out = renderDocs(g, { descriptions: store({ 'domain:core': coreRow }) });
    const agents = out.find((p) => p.kind === 'agents').content;
    expect(agents).toContain('## What the domains are for (model-written)');
    expect(agents).toContain('written by a language model');
    expect(agents).toContain('Shared primitives every other domain builds on.');
    expect(agents).toContain('Generated by `fake-model` via `command` on 2026-08-17');
  });

  it('labels the description on the domain page too, above the text', () => {
    const out = renderDocs(g, { descriptions: store({ 'domain:core': coreRow }) });
    const corePage = out.find((p) => p.path === 'domains/core.md').content;
    expect(corePage).toContain('Generated by `fake-model` via `command` on 2026-08-17');
    expect(corePage).toContain('_Shared primitives every other domain builds on._');
    expect(corePage.indexOf('Generated by')).toBeLessThan(corePage.indexOf('Shared primitives'));
  });

  it('leaves a domain with no description byte-identical', () => {
    const out = renderDocs(g, { descriptions: store({ 'domain:core': coreRow }) });
    const uiPage = out.find((p) => p.path === 'domains/ui.md').content;
    expect(uiPage).not.toContain('Generated by');
    expect(uiPage).toBe(page('domain', 'domains/ui.md').content);
  });

  it('labels descriptions in Russian too', () => {
    const out = renderDocs(g, { lang: 'ru', descriptions: store({ 'domain:core': coreRow }) });
    const agents = out.find((p) => p.kind === 'agents').content;
    expect(agents).toContain('написано моделью');
    expect(agents).toContain('Сгенерировано моделью `fake-model`');
  });

  it('stays byte-deterministic with descriptions in play', () => {
    const opts = { descriptions: store({ 'domain:core': coreRow }) };
    expect(renderDocs(g, opts)).toEqual(renderDocs(g, opts));
  });
});
